            ...
            <div class="content-wrapper">
                <div class="content-container">
                    ...
                    <div class="main-page">
                        ...
                        // Query to get the number of students for each ClassId, combining sections A and B
                        $query = "SELECT 
            CASE
                WHEN ClassId IN (1, 2) THEN 'Combined'
                ELSE (SELECT ClassName FROM tblclasses WHERE id = ClassId)
            END AS CombinedClassName,
            COUNT(*) AS num_students
            FROM tblstudents
            GROUP BY CombinedClassName";
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            $className = $row['CombinedClassName'];
                            $numStudents = $row['num_students'];
                            $dataPoints[] = array("y" => $numStudents, "label" => $className);
                        }

                        // Sort the data based on class order
                        usort($dataPoints, function ($a, $b) {
                            $classOrder = array('First Year', 'Second Year', 'Third Year', 'Fourth Year', 'Combined');
                            $classA = array_search($a['label'], $classOrder);
                            $classB = array_search($b['label'], $classOrder);
                                    ...
                                    <div class="col-lg-12">
                                        <div class="panel panel-default">
                                            <div class="panel-heading">
                                                <h3 class="panel-title">Student Distribution</h3>
                                            </div>
